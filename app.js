...
document.getElementById('btnPin').onclick = async ()=>{
  ...
  if(!snap.exists()){ await setDoc(refUser, { pinHash:h, createdAt:Date.now() }); alert('PIN creado. Acceso concedido.'); }
  else alert(snap.data().pinHash===h ? 'Acceso concedido.' : 'PIN incorrecto.');
};
...
/* ---------- ACCOUNTING: doble partida + estados ---------- */
...
function addLineRow() {
  ...
  tr.innerHTML = `
    <td><input placeholder="Código o nombre" class="acc"></td>
...
formTxn.addEventListener('submit', async(e)=>{
  ...
  if(lines.length<2 || totalD<=0 || totalC<=0 || totalD.toFixed(2)!==totalC.toFixed(2)) return alert('Asiento inválido (doble partida).');
...
function ledgerFromTransactions(trans){
  const map = new Map(CHART_OF_ACCOUNTS.map(a=>[a.code,{...a, debit:0, credit:0}]));
  const findType = (codeOrName)=>{
    const f = CHART_OF_ACCOUNTS.find(a=>a.code===codeOrName || a.name.toLowerCase()===codeOrName.toLowerCase());
    return f ? f.type : 'asset';
  };
  for(const t of trans){
    for(const l of t.lines){
      const key = l.account;
      if(!map.has(key)) map.set(key,{code:key, name:key, type:findType(key), debit:0, credit:0});
...
onAuthStateChanged(auth, (u)=>{
  ...
  onSnapshot(qTx, snap=>{
    ...
    // estados
    const ledger = ledgerFromTransactions(trans);
...
async function generateInvoicePDF(inv){
  ...
  return doc.output('arraybuffer'); // ArrayBuffer
}
...
function initGis(){
  tokenClient = google.accounts.oauth2.initTokenClient({
    client_id: 'TU_CLIENT_ID_OAUTH', // reemplaza
